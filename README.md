...
- config: 存放应用程序的配置文件。
  ...
  - local.yml: 本地环境的配置文件。
  - release.yml: 生产环境的配置文件。
- internal: 存放应用程序的内部代码。

  - controller: 处理HTTP请求的控制器。
    - controller.go: 处理HTTP请求的通用控制器,依赖注入配置。
    - user.go: 处理用户相关的HTTP请求的控制器。
  - middleware: 存放中间件代码。
    - cors.go: 跨域资源共享中间件。
    - logger.go: 接管默认日志中间件。
    - recovery.go: 接管默认恢复中间件。
  - repository: 存储库相关代码。
    - repository: 存储库相关代码,依赖注入配置
    - db: 存放数据库代码
      - db.go: 数据访问的通用接口。
      - user.go: 用户数据访问接口的实现自定义。
      - user_gen.go: 用户数据访问接口的实现系统生成。
  - router: 存放路由代码。
    - router.go: HTTP路由的实现,依赖注入配置。
    - v0: 默认第一个/公共版本。
      - user.go: 用户相关路由代码。
  - service: 存放业务逻辑代码。
    - service.go: 业务逻辑的通用接口,依赖注入配置。
    - user.go: 用户业务逻辑的实现。
- pkg: 存放应用程序的公共包。
- go.mod: Go模块文件。
- go.sum: Go模块的依赖版本文件。
...
这些命令将分别创建一个名为 `UserCtl` 和 `UserSvc`的组件，并将它们放置在正确的目录中。
