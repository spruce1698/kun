...
// (db/redis/kafka/tracer/xlog 等),避免 http 与 broker 各抄一份关闭序列、
// 新增资源要改两处。并发安全:Close 仅执行一次,重复调用直接返回。
//
// 用法:
//
//	c := xserver.NewCloser()
//	c.Add(func(ctx context.Context) error { return sqlDB.Close() }, "db")
//	c.Add(func(ctx context.Context) error { return redis.Close() }, "redis")
//	// 退出时:err := c.Close(ctx)  // 按 LIFO: redis -> db
type Closer struct {
	mu      sync.Mutex
	closers []closeEntry
	closed  bool
