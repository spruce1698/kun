 ...
 * @Desc: HTTP 应用装配层
 *
 * 作为 Composition Root，在 cmd/server/wire 中完成 gin 引擎与各层业务组件（中间件/JWT/限流/路由/Swagger）的组装，
...
type Assembly struct {
	Engine *gin.Engine
	// Closer 聚合进程退出时需回收的全部资源(db/redis/kafka/tracer/xlog),
	// 交给 http.Server 在 Stop 时按 LIFO 统一关闭。
	Closer *xserver.Closer
}

// NewHttp 装配 http 引擎并注册资源关闭器:
// gin 模式、中间件、JWT、限流、路由、Swagger,以及 db/redis/kafka/tracer/xlog 的回收。
...
func NewHttp(
	...
	// 期望关闭顺序:http(由 Server.Stop 直接关) -> db -> redis -> kafka publisher -> tracer -> xlog
	// 因此注册顺序:xlog(最先注册,最后关) -> tracer -> kafka -> redis -> db
