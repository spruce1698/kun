...
func (c *custom{{.StructName}}Db) ListWithTotal(ctx context.Context, args *{{.StructName}}Search) ([]*{{.StructName}}, int64, error) {
	...
	case offset > 100000 : // 深分页: SELECT * FROM demo INNER JOIN (SELECT id FROM demo WHERE ... ORDER BY id LIMIT ?,?) AS tmp USING({{.PrimaryKeyColumn}})
		// 子查询带过滤条件,JOIN 出的 {{.PrimaryKeyColumn}} 集合已是过滤后的;外层用独立的 Model 不带过滤,避免 WHERE 冗余。
	...
	{{- else -}}
	model =  filter().Offset(offset).Limit(limit)
	{{- end}}
